<template>
  <b-modal id="save-supplier-resource" ref="save-supplier-resource" title="Registrar recurso" size="lg" centered
    scrollable no-close-on-backdrop no-close-on-esc @shown="getResourceList" @close="cleanForm">
    <template v-slot:modal-header="{ close }">
      <h5 class="modal-title text-darker-secondary my-0">Registrar recurso</h5>
          ...
          <b-row>
            <b-col cols="12">
              <b-form-group>
                ...
                <b-form-input id="name" data-cy="input-supplier-resource-name" type="text" placeholder="Nombre..."
                  maxlength="50" v-model="$v.resource.name.$model"
                  :state="$v.resource.name.$dirty ? !$v.resource.name.$error : null" @blur="$v.resource.name.$touch()">
                </b-form-input>
            ...
            <b-col cols="12">
              <b-form-group
                ...
                <multiselect id="resource" data-cy="multiselect-save-supplier-resource-resource"
                  :config="multiselectConfig" @update:modelValue="$v.resource.resource.$model = $event">
                </multiselect>
            ...
            <b-col cols="12">
              <b-form-group>
                ...
                <b-form-input id="unitMeasure" data-cy="input-supplier-resource-unit-measure"
                  placeholder="Unidad de medida..." maxlength="30" v-model="$v.resource.unitMeasure.$model"
            ...
            <b-col cols="12">
              <b-form-group>
                ...
                <b-form-textarea id="description" data-cy="textarea-supplier-resource-description"
                  placeholder="Descripción..." rows="4" maxlength="250" v-model="$v.resource.description.$model"
          ...
          <b-row>
            <b-col cols="12">
              <b-form-group>
                ...
                <div class="image-selector-container" @click="triggerMainImageFileInput" v-if="!resource.mainImage">
                  <template>
                    <b-row>
                      <b-col cols="12" class="d-flex justify-content-center align-items-center text-center flex-column">
                        <svg-icon type="mdi" size="2em" class="img-icon text-dark-secondary" :path="$icons.mdiImage" />
                    ...
                    <input data-cy="input-save-supplier-resource-image" type="file" id="main-image" ref="mainImage"
                      class="d-none" accept="image/*" @change="selectMainImage" />
                  </template>
                </div>
                <div v-else>
                  <template>
                    <div class="selected-image">
                      ...
                      <div class="icon-remove">
                        <b-button data-cy="button-save-supplier-remove-image" variant="danger" size="sm"
                          class="btn btn-rounded" @click="removeMainImage" v-b-popover.hover.top="'Remover imagen'">
                          <svg-icon type="mdi" size="1em" class="text-white" :path="$icons.mdiClose" />
                        </b-button>
                      </div>
                    </div>
                  </template>
            ...
            <b-col cols="12">
              <b-form-group>
                ...
                <b-row>
                  <b-col cols="6" v-for="(image, index) in resource.secondaryImages" :key="'selected-image-' + index">
                    <div class="selected-secondary-image">
                      <img :src="image" alt="Recurso" />
                      <div class="icon-remove">
                        <b-button data-cy="button-save-supplier-remove-secondary-image" variant="danger" size="sm"
                          class="btn btn-rounded" @click="resource.secondaryImages.splice(index, 1)"
                  ...
                  <b-col cols="6" v-for="index in 2 - resource.secondaryImages.length" :key="'empty-state-' + index">
                    <div class="no-secondary-image">
                      <img :src="require('@/assets/img/no-image-found.svg')" alt="empty-state" />
                    </div>
                  </b-col>
                </b-row>
                <b-button data-cy="button-save-supplier-resource-secondary-image" variant="outline-primary"
                  class="d-flex justify-content-between align-items-center" @click="triggerSecondaryImageFileInput"
                ...
                <input data-cy="input-save-supplier-resource-secondary-image" type="file" id="secondary-images"
                  ref="secondaryImages" class="d-none" accept="image/*" @change="selectSecondaryImage"
                  :disabled="resource.secondaryImages.length >= 2" />
              </b-form-group>
            </b-col>
          </b-row>
        </b-col>
    ...
    <template #modal-footer="{ close }">
      <div class="d-flex justify-content-end">
        <b-button data-cy="button-save-supplier-resource" variant="primary" type="button" class="button-actions"
          :disabled="$v.$invalid" @click="$bvModal.show('save-supplier-resource-confirmation')">
          Registrar
        </b-button>
        <b-button data-cy="button-cancel-save-supplier-resource" variant="secondary"
          class="ml-2 button-actions text-white" type="button" @click="close">
          Cancelar
        </b-button>
      </div>
    </template>
    <confirmation-modal :config="confirmationConfig" @onConfirm="saveSupplierResource" />
  </b-modal>
</template>
...
export default Vue.extend({
  name: 'SaveSupplierResourceModal',
  props: {
  ...
  data() {
    return {
      ...
      confirmationConfig: {
        id: 'save-supplier-resource-confirmation',
        type: 'question',
    ...
    async saveSupplierResource() {
      try {
        ...
        const payload = {
          ...
          ...this.resource
        } as SaveSupplierResourceDto;
        const controller = new SuppliersController();
        const { status } = await controller.saveSupplierResource(payload);
        if (status === 201) {
          showSuccessToast('Recurso registrado');
          this.$bvModal.hide('save-supplier-resource');
          this.cleanForm();
          this.$emit('reload');
        }
      } catch (error) {
        ...
        this.$bvModal.hide('save-supplier-resource-confirmation');
      }
    },
    cleanForm() {
      this.$v.$reset();
